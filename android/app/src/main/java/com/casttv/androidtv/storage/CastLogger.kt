...
object CastLogger {
    ...
    private var _cachedPreviousEntries: List<LogMessage>? = null
    ...
    var onNewEntry: ((LogMessage) -> Unit)?
        ...
        set(value) { lock.withLock { _onNewEntry = value } }
    ...
    fun previousSessionEntries(): List<LogMessage> {
        lock.withLock { _cachedPreviousEntries }?.let { return it }

        val file = logFile ?: return emptyList()
        if (!file.exists()) {
            lock.withLock { _cachedPreviousEntries = emptyList() }
            return emptyList()
        }
        ...
        lock.withLock { _cachedPreviousEntries = result }
        return result
    }
    ...
    fun clearAll() {
        lock.withLock {
            entries.clear()
            _cachedPreviousEntries = null
        }
        try { logFile?.delete(); logFile?.createNewFile() } catch (_: Exception) {}
    }

    private fun writeToFile(entry: Entry) {
        val file = logFile ?: return
        try {
            val json = "{\"timestamp\":\"${entry.toIso8601()}\",\"level\":\"${entry.level}\",\"message\":${JSONObject.quote(entry.message)}}\n"
    ...
    private fun Entry.toLogMessage() = LogMessage(
        timestamp = toIso8601(),
        level = level,
        message = message
    )
