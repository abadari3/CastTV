...
class PlayerActivity : ComponentActivity() {
        ...
        val loadControl =
            androidx.media3.exoplayer.DefaultLoadControl.Builder()
                .setBufferDurationsMs(
                    // min buffer 20s (keep buffer consistently filled)
                    20_000,
                    // max buffer 20s (same as min — drip-feeding for stable playback)
                    20_000,
                    // 1s buffer before first frame (faster start)
                    1_000,
                    // 1s buffer after rebuffer (faster recovery)
                    1_000,
                )
    ...
    private fun seekIncrement(repeatCount: Int): Long =
        when {
